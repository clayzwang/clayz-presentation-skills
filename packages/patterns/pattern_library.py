...
def resolve_composition_pattern(
    ...
    *,
    ...
    retrieval_request = {
        ...
        "task_context": {"decision_goal": "Resolve the most relevant composition pattern", "target_refs": [f"slide-role:{normalized['page_role']}", "decision:composition-pattern"], "format_need": "composition-pattern"},
    ...
    if selected_id:
        ...
        failure_request = {
            ...
            "task_context": {"decision_goal": "Resolve linked failure guards for the selected pattern", "target_refs": [f"slide-role:{normalized['page_role']}", "decision:failure-guard"], "format_need": "failure-pattern"},
            "ranking_policy": {"profile": "failure", "minimum_score": 0.1, "max_selected": min(len(linked_failure_ids) or 1, 10), "diversity_lambda": 0.8},
            "rights_context": normalized["rights_context"],
            "require_human_admission": True,
            "limit": 50,
            "filters": {
