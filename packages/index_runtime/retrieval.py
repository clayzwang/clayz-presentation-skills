...
class CompositeIndex:
    ...
    def _search_text(record: Mapping[str, Any]) -> str:
        ...
        return "\n".join(
            [
                ...
                record["summary"],
                record["record_type"],
                " ".join(classification["task_modes"]),
                " ".join(classification["page_roles"]),
                " ".join(classification["semantic_relations"]),
                " ".join(classification["purpose_tags"]),
                " ".join(classification["failure_signals"]),
            ]
        )
            ...
            ranked.append(
                {
                    ...
                    "never_copy": list(record["rights"].get("never_copy", [])),
                    "neighbor_of": None,
                    "neighbor_type": None,
                }
            )
                    ...
                    expanded.append(
                        {
                            ...
                            "rights_decision": rights_decision,
                            "materializable": materializable,
                            "source_id": neighbor["source"]["source_id"],
                            "source_revision": neighbor["source"]["source_revision"],
                            "license": neighbor["rights"]["license"],
                            "never_copy": list(neighbor["rights"].get("never_copy", [])),
                            "neighbor_of": result["record_id"],
                            "neighbor_type": field,
                        }
                    )
    ...
    def finalize_receipt(
        ...
        *,
        ...
        finalized["fallback"] = {
            "used": not bool(selected),
            "reason": "no-record-selected" if finalized.get("candidates") and not selected else finalized["fallback"].get("reason", ""),
        }
        return finalized
