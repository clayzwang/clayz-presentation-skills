...
## 0.10.1 same-name global Skill upgrade

The `clayz-presentation-personal` Skill can be invoked in ordinary Chat or Work
without a ChatGPT Project. The existing host Library root is a resource binding,
not a project dependency. It routes inspection, discussion learning, and PPT
work before selecting production prerequisites.

The composer includes shared discussion/snapshot code, `cloud_learning_cli.py`,
and a native Library policy derived from the existing host-library mount.
Confirmed knowledge is an optional supplemental IndexProvider under that
Library's `_extension/confirmed-learning`; existing private providers and brand
bindings remain intact. See [the native workflow](../packages/contracts/native-library-workflow.md).
Local staging produces an awaiting-host-write plan. Only actual host writes and
fresh Library readback can establish persistence; there is no bundled remote API
client, NAS redirection, or claim of authenticated/distributed transactions.
...
The standalone upload archive also excludes `.github`, READMEs, the Changelog, the empty knowledge scaffold, and other repository-maintenance files, and retains only one stage-reference tree. Each internal `stage.md` routes to the single copy under `skills/<stage>/references/` instead of duplicating the same references under `references/stages/<stage>/references/`. This reduces upload members and bytes without changing the Public Core, Provider snapshots, or stage methods.

The composer prunes locale-parallel prose only when it is not an executable dependency. It retains the selected-language stage contracts, public Capability Index references, and every file in the component-version reader's shared dependency table. The genuine English Copy contract remains alongside the Chinese reading version because its version marker is read by code; do not rename translated text to impersonate that source. The existing runtime lock records SHA-256 hashes for every delivered file except itself. Packaging reopens and extracts the final ZIP and runs `--mode archive`, requiring the exact file set and all hashes. At runtime use the default `--mode installed`: ChatGPT regenerates `agents/openai.yaml` and may manage `assets/icon.svg`; these UI files and extra host files are observations, not runtime authority or authoring blockers. All other declared files, required dependency coverage, actual component versions and private configuration locks remain strict. Task outputs should live outside the Skill directory. The 192-member target is conservative upload guidance, not a proven platform limit or permission to drop dependencies.
...
## Long-context visibility and resume

The Personal Skill must not repeatedly place complete five-stage JSON, retrieval bodies, logs, and audit reports back into the conversation. The root Skill sends one short visible progress update before the first long scan. After preflight and each stage it reports only the gate completed, the next gate, and any blocker or deferred acceptance; full evidence remains in task files. The model loads only the root rules, the current stage module, locale-matched references needed for the current decision, and path/SHA-256 pointers to approved artifacts. It reopens required fields from those files on demand instead of restating earlier artifacts. Long inputs and selected Library records are read by relevant excerpt or chunk, never loaded in full by default.

This compresses repeated transport and execution, not Logic reasoning. Each stage starts with one focused retrieval and adds another receipt only for an unresolved material question; candidate and selection budgets are cumulative for the stage. Logic may fully use owner-private facts, methods, and counterexamples but must independently synthesize where the audience is now, where it must go, and how it gets there. Unless the user explicitly omits both, cover and closing remain boundary narrative roles and body-slide count is separate from total slide count. The Storyline itself is populated and positioned according to the active master; any smaller supporting copy beneath it is optional and remains empty when it adds no independent information.

After preflight and every stage, the task root refreshes a small `run-context-checkpoint.json`. It contains only the run and request hashes, completed and next phase, authoritative artifact paths and hashes, blockers or deferred checks, and a concise requirement summary. It is a derived resume index rather than a parallel contract source, and it never contains private source text or chain-of-thought. When the conversation already includes a failed full-deck attempt, repeated large generations, or a host context/compaction warning, validate that checkpoint and resume from `next_phase` without rerunning hash-valid stages. If the next stage cannot be loaded safely, stop before starting it at the durable checkpoint and ask the user to continue in a fresh chat with the checkpoint and authoritative artifacts. This is a resumable context boundary, not a failed presentation run.

Once the paired delivery directory passes validation, the chat response is delivery-first and compact: provide the PPTX and supervision report, state validation status and only material deferred checks, and leave the full audit in the report file. This keeps generated files, evidence, and the resume point recoverable even when a long conversation or mobile client becomes unstable.
