...
2. Final rendering in the v0.7.0 local release: one PowerPoint COM process on Windows. Cloud Public Light may select a host-provided presentation or Artifact Tool route during preflight. Other local operating-system routes are not v0.7.0 release claims.
