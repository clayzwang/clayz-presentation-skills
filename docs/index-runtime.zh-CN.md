...
阶段检索是实质执行证据，不是枚举资料源的仪式。每次新请求都必须包含非空任务问题、决策目标、格式需要、评分档、阈值、选择上限和多样性设置。候选给出0到1的总分，以及语义、阶段、任务语境、关系/目的、格式适配和证据质量分项。最终选择必须指向具体采用目标，并标明采用是计划中还是已经实质发生。

每个治理阶段最多三份回执、六十个候选。必需来源可以合并消费，但阶段仍须提供面向当前决策的相关性证据。空查询、只有资料源名称而没有决策目标、低于阈值的选择、泛化理由或没有采用目标的选择均失败关闭。
