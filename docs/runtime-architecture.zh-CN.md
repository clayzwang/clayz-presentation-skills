...
`自然语言请求 → 已安装组件离线一致性检查 → 本版本首次私有学习或哈希复用 → Supervisor签发新鲜任务挑战并绑定配置 → 一次预检与资源简报 → 锁定路线/来源 → Logic → Copy → Art Direction → Output构建 → Supervisor最终渲染审计 → 最多一次定向修复 → 经校验的成对发布目录`

运行中不换路线。后端发生硬失败时结束本次运行；中央配置最多允许从新预检开始一次备选路线重启。

应用端每次任务只离线核对已安装 Public Core、插件/配置、工作流、运行时、五阶段合同与 Personal Runtime 的内部一致性。组件缺失、损坏或内部不兼容仍阻止制作；GitHub 版本差异、无法联网和开发候选日期过期不阻止。运行时预检绑定15分钟内生成的安装检查报告；跨端发布核验由开发端显式运行 --mode release-check。

owner-personal 模式下，`scripts/bootstrap_owner_learning.py` 按公共核心和已准入来源修订建立持久索引。未变化来源复用解析缓存，新准入的来源集合建立新快照，旧任务保留原快照；旧修订的字节漂移会被拒绝。[本地讨论流程](plugin-system.zh-CN.md)把用户确认的共识及附件转为同一 Index 引擎可检索的私有快照。这里的学习是知识持久化，不是修改模型权重。资源检查和资料讨论走独立入口，不要求先启动 PPT 制作或渲染器。
