...
v0.8.0 保留 v0.5.2 对演示推理、受治理检索与执行的分离，并把 Logic 前的资源盘点升级为强制步骤。唯一 Public Core 与随包公共 Provider 在所有目标中构成同一个脑；本地适配器执行 Local Light 路线，ChatGPT 宿主工具构成 Cloud Light 的身体。五个 Skill 继续拥有内容和设计决策；Runtime 负责资源与能力发现、路线锁定、有界工具调用和宿主边界。

## 固定生命周期

`自然语言请求 → 官方最新版本与组件表门禁 → 本版本首次私有学习或哈希复用 → Supervisor签发新鲜任务挑战并绑定配置 → 一次预检与资源简报 → 锁定路线/来源 → Logic → Copy → Art Direction → Output构建 → Supervisor最终渲染审计 → 最多一次定向修复 → 经校验的成对发布目录`

运行中不换路线。后端发生硬失败时结束本次运行；中央配置最多允许从新预检开始一次备选路线重启。

组件版本门禁在每次任务启动时访问官方 GitHub Latest Release，并逐项核对 Public Core、插件/配置、工作流、运行时、五阶段合同与 Personal Runtime。Supervisor 必须先把版本表打印给用户；无法联网核验、组件缺失、版本落后或同包漂移都在 Logic 前失败关闭。运行时预检还会再次绑定15分钟内生成的版本报告，避免只在提示词里声称检查完成。

owner-personal 模式下，`scripts/bootstrap_owner_learning.py` 以 Public Core 版本和真实私有来源集合哈希建立一次性学习键。某版本首次运行时读取已准入的知识、模板、规范和方法原始字节，构建索引、执行检索探针，并生成独立 JSON/Markdown 学习审计。后续任务只验证并复用同一索引；同版本来源变化会失败关闭，不能静默覆盖首次审计。这里的学习是持久化、可检索、可审计的私有索引学习，不是修改模型权重。
...
2. v0.8.0 本地发布的最终渲染：Windows 使用一个 PowerPoint COM 进程。Cloud Public Light 可在预检时选择宿主提供的演示或 Artifact Tool 路线。其他本地操作系统路线不是本版发布承诺。
