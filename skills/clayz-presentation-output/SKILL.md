...
5. Read `references/expression-mode.md`, `references/relative-layout.md`, and `references/layout-contract-compilation.md`; they define approved-medium implementation, constraint-based regions, and the registered-contract compilation boundary.
...
8. Load only optional `knowledge_refs` returned by selected capability records. Keep capability and Layout Contract resolution and retrieval receipt IDs as task-local evidence; unresolved signals never justify pretending renderer support or inventing a repair method.
