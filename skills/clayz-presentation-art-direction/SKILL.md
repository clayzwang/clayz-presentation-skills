...
3. Validate the copy-approved package and inherit its resource-inventory signature. Use only selected pools or assets; a new template, image, font, icon, or reference source must return to Supervisor for a revised inventory and user brief.
...
7. Read the locale-matched `../clayz-presentation-supervisor/references/first-class-index-gate.md`. Reuse the one task Provider lock created before Logic. In owner-personal mode, Art Direction may not start until every owner-learning source declared for Art Direction by the task manifest is hashed into `task-private-learning` and selected through finalized receipts.
...
14. Emit one plan with `origin_namespace: io.clayz.presentation`, status `art-direction-approved`, root `resource_inventory_lock`, and root `index_evidence`. `reference_budget.query_log` must bind every loaded record to an Art Direction receipt and adoption outcome. Generic first visuals such as “主图/主表/结论结构”, non-series silhouette or structure reuse, and multi-value shape pseudo-charts fail validation.
