...
2. Run `../../scripts/component_version_guard.py --output <component-version-report.json> --brief-output <component-version-brief.md>` before any other presentation work. The guard must obtain the official GitHub Latest Release response during this run, either directly or from an exact host-materialized response passed with `--latest-release-json`. Print the full concise version table from `user_brief` in commentary: public core, plugin/config, workflow/runtime, five stage contracts, Personal Runtime core/Profile when present, and the official latest version. Any `blocked` result, unavailable latest-release evidence, older core, missing component, or component drift is `non-latest-presentation-component`; stop before Logic. A cached success from another task is not evidence for this task.
