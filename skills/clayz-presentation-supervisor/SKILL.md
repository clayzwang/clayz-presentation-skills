...
3. Resolve and validate the Personal Extension Runtime before Logic. In an owner-personal task, absence, invalidity, or hash drift is `personal-extension-not-loaded`; do not silently fall back to the public theme unless the user explicitly requests public-core mode.
4. Resolve every declared Provider manifest through the runtime mount, validate its identity and locked snapshot, and record one task Provider lock. Required Provider or brand-asset failure is fail-closed. Optional Provider failure must be recorded as an explicit public-core fallback.
...
6. Run one host capability preflight, lock the authoring/rendering route, finalize `ppt-resource-inventory.json`, and send its concise user-facing brief in commentary. State what was found, what this task will use, what is unavailable or not selected, and the locked route. If ready, continue automatically; no governed stage may begin before this message.
...
8. After Output, resume the independent audit workflow below. Delivery is not authorized until a supervision report exists, upstream validators are rerun against the final PPTX, selected resources are reconciled to actual use, and all blocking findings are resolved or explicitly accepted by the user.
...
1. Verify the root orchestration checkpoint and bind the plugin-mount report plus exact hashes of the pre-Logic resource inventory, Logic package, Copy package, Art Direction plan, PPTX, render root, object inventory, deviation log, QA report, resolved configuration, Personal Extension Runtime, Provider lock, route lock, and available retrieval receipts.
...
10. Emit one report with `origin_namespace: io.clayz.presentation`, status `supervised`, cumulative root `index_evidence`, and validated `resource_usage`. Every per-slide check evidence must cite the stable slide ID and be unique to that check; repeated boilerplate is invalid. A missing report proves Supervisor was not invoked and blocks normal delivery.
