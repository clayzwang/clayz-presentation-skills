...
- 核心脚本需要Python 3.10或更高版本；位图准备、视觉节奏和文件体积检查使用Pillow。
...
- 当前仓库提供Skill、合同、集中配置、验证器和签名工具，尚不是一条命令即可调用任意模型的独立Runner。
...
PPTAgent、DeepPresenter、pom、VASCAR 与 PptxGenJS 仅作为明确标识的概念启发或可选运行时路线，准确链接、许可和影响范围记录在 `provenance/manifest.yaml`。仓库不打包其源码快照、模板、媒体或模型，也不包含任何上传的母版、PPT、PDF、字体、截图或其衍生素材；`examples/` 全部为合成数据。
